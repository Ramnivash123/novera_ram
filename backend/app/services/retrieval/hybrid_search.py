    ...
    async def search(
        ...
        if use_semantic:
            ...
            for idx, chunk in enumerate(semantic_results, 1):
                ...
                results[chunk_id]['semantic_score'] = chunk[
                    'similarity_score'
                ]
        ...
        if use_keyword:
            ...
            keyword_results = (
                await keyword_search_service.search_keywords(
                    query=query,
                    db=db,
                    top_k=k,
                    doc_type=doc_type,
            ...
            for idx, chunk in enumerate(keyword_results, 1):
                ...
                results[chunk_id]['keyword_score'] = chunk.get(
                    'keyword_score', 0
                )
        ...
        logger.info(
            f"Hybrid search completed: {len(sorted_results)} final results"
        )
    ...
    def _reciprocal_rank_fusion(
        ...
        for chunk_id, chunk in results.items():
            ...
            if boost_documents:
                ...
                is_boosted = any(
                    boost_doc.lower() in doc_name.lower()
                    or doc_name.lower() in boost_doc.lower()
    ...
    async def search_with_context_expansion(
        ...
        for result in initial_results[:5]:
            ...
            try:
                ...
                logger.warning(
                    f"Invalid chunk ID format: {chunk_id_raw}, error: {e}"
                )
                continue
            ...
            for neighbor in neighbors:
                ...
                if nid not in seen_chunk_ids:
                    neighbor['is_expanded_context'] = (
                        not neighbor['is_target']
                    )
        ...
        logger.info(
            f"Context expansion: {len(initial_results)} → "
